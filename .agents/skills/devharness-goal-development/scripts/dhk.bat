...
if not exist "%JAR%" (
  set "TARGET_JAR="
  for %%F in ("%PROJECT_ROOT%\target\dhk-cli-*-all.jar") do if not defined TARGET_JAR if exist "%%~fF" set "TARGET_JAR=%%~fF"
  if defined TARGET_JAR set "JAR=%TARGET_JAR%"
)
if not exist "%JAR%" (
...
for %%A in (%*) do (
  if "%%~A"=="--project-root" set "HAS_PROJECT_ROOT=1"
