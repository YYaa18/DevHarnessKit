...
- Workflow artifacts, exported memory bindings, and checkpoint bindings provide an audit trail for each run.
...
- Phase and gate commands must target the run's current phase.
...
- Use `workflow artifact list` and `workflow summary` to inspect what context and memory a run used.
- Bind checkpoints explicitly with `workflow bind-checkpoint` when a checkpoint belongs to a run.
