...
6. Confirm archive contents and wrapper contracts:
   ...
   unzip -l "$DHK_ZIP" | grep 'README.md'
   unzip -l "$DHK_ZIP" | grep 'LICENSE'
   unzip -l "$DHK_ZIP" | grep 'THIRD_PARTY_NOTICES.md'
   unzip -l "$DHK_ZIP" | grep 'SECURITY.md'
   unzip -l "$DHK_ZIP" | grep 'CHANGELOG.md'
   ...
   ! unzip -l "$DHK_ZIP" | grep 'docs/'
   ! unzip -l "$DHK_ZIP" | grep 'RELEASE.md'
   ! unzip -l "$DHK_ZIP" | grep 'CONTRIBUTING.md'
   ! unzip -l "$DHK_ZIP" | grep 'CODE_OF_CONDUCT.md'
   ! unzip -l "$DHK_ZIP" | grep 'scripts/check-version-metadata.sh'
   ! unzip -l "$DHK_ZIP" | grep 'scripts/release-gate.sh'
   ! unzip -l "$DHK_ZIP" | grep 'scripts/perf-smoke.sh'
   ! unzip -l "$DHK_ZIP" | grep 'scripts/lark-codex-bridge.sh'
