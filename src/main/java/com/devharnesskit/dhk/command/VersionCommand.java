...
public final class VersionCommand implements Command {
    public int run(CommandContext context, Args args) {
        ...
        context.out().println("stable_surface: help,version,doctor,memory-core,goal-core,bdd,configure,status,readiness,context-artifacts,advise,quickstart,release-packaging");
        context.out().println("stable_candidate_surface: context-governor,goal-retrospective,skill-contract,policy-hooks,routine-report");
        context.out().println("beta_surface: db-readonly");
        context.out().println("experimental_surface: graph-aware-goal,context-headroom,skill-governance-full,policy-governance-full,ecc-control-panel,routine");
