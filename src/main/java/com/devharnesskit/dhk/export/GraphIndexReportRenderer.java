...
public final class GraphIndexReportRenderer {
    public String render(GraphScanReport report, Instant generatedAt) {
        ...
        if (report.currentWorkspaceFingerprint().length() > 0) {
            builder.append("- current_workspace_fingerprint: ").append(report.currentWorkspaceFingerprint()).append('\n');
        }
        if (report.latestSnapshot() != null) {
            ...
            builder.append("- latest_snapshot_workspace_fingerprint: ")
                    .append(report.latestSnapshot().workspaceFingerprint()).append('\n');
            builder.append("- latest_snapshot_stale: ").append(report.latestSnapshotStale()).append('\n');
